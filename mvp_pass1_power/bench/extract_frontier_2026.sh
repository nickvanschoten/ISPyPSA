...
OUT=mvp_pass1_power/outputs/frontier_2026
...
for cp in 0 40 80 150 250 350 550; do
  ...
  PYTHONPATH=. uv run python mvp_pass1_power/postprocess/extract_frontier_points.py \
    --sweep-id c${cp} --run-id "${RUNID[$cp]}" --years 2030 2035 2040 2045 2050 \
    --carbon-price ${cp} --tns-price 0 \
    --workbook-cache mvp_pass1_power/data/workbook_cache_v75 --out-dir "${OUT}" \
